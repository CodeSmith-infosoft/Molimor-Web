...
export default function CategoryDropdown({ categoriesData }) {
  ...
  const renderCategoryItem = (item, level = 0) => {
    ...
    return (
      <div key={item.id}>
        <div
          className={`flex cursor-pointer items-center justify-between px-[18px] py-[13px] cursor-pointe`}
  ...
  return (
    <div className="relative">
      <button
        ...
        onMouseEnter={() => setIsMainOpen(true)}
        onMouseLeave={() => setIsMainOpen(false)}
        className="flex items-center cursor-pointer gap-2 text-[15px] px-[18px] py-[14px] w-[298px] justify-between hover:text-green transition-all ease-in-out duration-200"
      >
      ...
      {isMainOpen && (
        <div
          className="absolute hide-scrollbar max-h-[601px] top-11 left-0 mt-1 bg-white max-w-[298px] z-20 w-80 overflow-y-auto"
          onMouseEnter={() => setIsMainOpen(true)}
          onMouseLeave={() => setIsMainOpen(false)}
        >
          <div className="py-2">
